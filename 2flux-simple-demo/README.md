...
个人总结：  
mvc框架不同模块之间的依赖关系，使得巨大的代码库和庞大的组织系统变得“脆弱且不可预测”。每当新增一个功能时，对代码的修改很容易引入新的bug。对于刚刚加入新手更是举步维艰。因为不知道修改后的代码会造成什么样的后果。因此facebook推出了flux，flux的特点是*更严格的数据流控制（单向数据流动）*。  
flux的不足：  
1、store之间的依赖关系  
在flux体系中，如果两个store之间有逻辑依赖关系，就必须用上Dispatch的waitFor函数告诉dispatch先处理哪个action对象。
2、很难进行服务器端渲染  
3、store混杂了逻辑和状态  
当需要动态替换一个store的逻辑时，只能把这个store整体替换掉，就无法保持store中存储的状态。
